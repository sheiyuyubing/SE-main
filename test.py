...
for method in method_list:
    ...
    for data in test_data:
        ...
        if len(audio_enhanced) < len(audio_clean):
            audio_enhanced = np.pad(audio_enhanced, (0, len(audio_clean) - len(audio_enhanced)), 'constant')
        elif len(audio_enhanced) > len(audio_clean):
            audio_enhanced = audio_enhanced[:len(audio_clean)]
